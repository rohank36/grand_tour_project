...
class Config:
    ...
    eval_every: int = 5
    eval_freq: int = int(1e4)  # Frequency in training steps (like cql.py)
    # general params
    train_seed: int = 0
    eval_seed: int = 27
    normalize_online_eval_obs: bool = False  # Normalize online eval obs
    device: str = "cuda"  # Device for PyTorch wrapper
...
def normalize_states_jax(states: jax.Array, mean: jax.Array, std: jax.Array) -> jax.Array:
    """Normalize JAX arrays"""
    return (states - mean) / std
...
class ReplayBuffer:
    ...
    def create_from_hdf5(
        ...
        # Create next_actions by shifting actions by one position
        next_actions = np.roll(dataset["actions"], shift=-1, axis=0)
        # When episode ends (dones[i] == True), next_action should be the same as current action
        terminals = dataset["terminals"]
        next_actions[terminals.astype(bool)] = dataset["actions"][terminals.astype(bool)]
        # Handle last transition
        next_actions[-1] = dataset["actions"][-1]
        ...
        buffer = {
            ...
            "next_states": jnp.asarray(dataset["next_observations"], dtype=jnp.float32),
            "next_actions": jnp.asarray(next_actions, dtype=jnp.float32),
            "dones": jnp.asarray(dataset["terminals"], dtype=jnp.float32),
        }
        if is_normalize:
            # Use numpy version from utils
            mean_np, std_np = compute_mean_std(dataset["observations"], eps=1e-3)
            self.mean = jnp.asarray(mean_np, dtype=jnp.float32)
            self.std = jnp.asarray(std_np, dtype=jnp.float32)
            buffer["states"] = normalize_states_jax(buffer["states"], self.mean, self.std)
            buffer["next_states"] = normalize_states_jax(
                buffer["next_states"], self.mean, self.std
            )
        else:
            self.mean = jnp.array(0.0)
            self.std = jnp.array(1.0)
        if normalize_reward:
            # Skip reward normalization for now (was dataset-specific)
            pass
        self.data = buffer
...
# PyTorch wrapper for JAX actor to work with OnlineEval
class JAXActorWrapper(nn_torch.Module):
    """Wrapper to convert JAX/Flax actor to PyTorch module for OnlineEval"""
    def __init__(self, jax_actor: TrainState, action_fn: Callable):
        super().__init__()
        self.jax_actor = jax_actor
        self.action_fn = action_fn
    
    @torch.no_grad()
    def act_inference(self, observations: torch.Tensor) -> torch.Tensor:
        """Convert PyTorch observations to JAX, get actions, convert back"""
        # Convert to numpy then to JAX
        obs_np = observations.cpu().numpy()
        obs_jax = jnp.asarray(obs_np)
        
        # Get actions from JAX actor
        actions_jax = self.action_fn(self.jax_actor.params, obs_jax)
        
        # Convert back to PyTorch tensor
        actions_np = np.asarray(jax.device_get(actions_jax))
        actions_torch = torch.from_numpy(actions_np).to(observations.device).float()
...
def train(config: Config):
    ...
    buffer = ReplayBuffer()
    buffer.create_from_hdf5(
        config.dataset_filepath, config.normalize_reward, config.normalize_states
    )
    ...
    for epoch in trange(config.num_epochs, desc="ReBRAC Epochs"):
        ...
        total_steps += config.num_updates_on_epoch
        wandb.log(
            {"epoch": epoch, "step": total_steps, **{f"ReBRAC/{k}": v for k, v in mean_metrics.items()}}
        )

        # Evaluate using OnlineEval (similar to cql.py)
        if total_steps % config.eval_freq == 0 or epoch == config.num_epochs - 1:
            # Create PyTorch wrapper for JAX actor
            actor_wrapper = JAXActorWrapper(update_carry["actor"], actor_action_fn)
            actor_wrapper.eval()
            
            result = online_eval.eval_actor_isaac(actor=actor_wrapper, device=config.device)
            if len(result) == 5:
